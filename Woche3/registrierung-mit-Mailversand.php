<?php
require 'PHPMailer/vendor/autoload.php'; // Mailer funktion laden
...
if( isset($_POST['prename']) && isset($_POST['surname']) ){
  ...
  if($hasError == false){
    ...
    // Mail an den registrierten User: 
    $mailer->setFrom('citystrolch@gmail.com', 'Terry Harker Privat');
    $mailer->addAddress($email, $prename.' '.$surname); // empfänger aus dem Formular
    
    $mailer->isHtml(true); // verwende HTML
    
    // Mail erstellen: 
    $mailer->Subject = 'Bestätigung deiner Registrierung auf WDD323';
    $mailer->Body = "Hallo $prename $surname, du bist jetzt registriert und kannst dich mit deinem Usernamen und Passwort anmelden.";
    $mailer->send();
    
    $mailer->clearAddresses(); // Empfänger-Array leeren, bevor wir ein neues E-Mail erstellen (sonst wird die nächste E-Mail an beide Empfänger geschickt)

    // Mail an den Admin: 
    $mailer->setFrom('citystrolch@gmail.com', 'Terry Harker Privat');
    $mailer->addAddress('terry@bytekultur.net', 'Terry'); // empfänger aus dem Formular

    // Mail erstellen: 
    $mailer->Subject = 'Neuer User';
    $mailer->Body = "Hallo Admin, Der User $prename $surname hat sich gerade registriert.";
    $mailer->send();
  }
